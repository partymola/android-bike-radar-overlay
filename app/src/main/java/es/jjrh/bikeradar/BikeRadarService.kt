...
class BikeRadarService : Service() {
    ...
    // Crash checkpoint for the in-flight ride: all lifecycle logic lives in
    // RideCheckpointCoordinator (see RideCheckpoint.kt); the service only
    // wires its four one-line calls into onCreate and the walk-away tick.
    private val rideCheckpoint = RideCheckpointCoordinator(
        store = RideCheckpointStore({ getExternalFilesDir(null) }),
        appendToHistory = { rideHistory.append(it) },
        journal = { linkJournal.log(it) },
    ...
    override fun onCreate() {
        ...
        // so a hold or a call has to push a repost rather than be read on the
        // next one.
        ...
        for (action in outcome.actions) {
            when (action) {
                ...
                is RideSummaryEdgeTracker.Action.PostSummary -> {
                    ...
                    rideCheckpoint.onSummaryPosted(posted)
                }
            }
        }
        rideCheckpoint.tick(
            snapshot = { rideStats.snapshot() },
