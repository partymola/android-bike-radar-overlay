...
/** Unit tests for [EnablingSequence.buildDeviceIdSuffix] - the phone-side
...
class EnablingSequenceDeviceIdTest {
    ...
    @Test fun `golden payload matches the original hardcoded constant`() {
        assertEquals(
            ...
            EnablingSequence.buildDeviceIdSuffix("bikeradar overlay", "Google", "Pixel 10 Pro XL"),
        )
    }

    @Test fun `empty vendor and model emit zero-length fields`() {
        val suffix = EnablingSequence.buildDeviceIdSuffix("bikeradar overlay", "", "")
        assertEquals(
    ...
    @Test fun `over-cap model field is truncated to 32 bytes`() {
        val suffix = EnablingSequence.buildDeviceIdSuffix("bikeradar overlay", "x", "a".repeat(40))
    ...
    @Test fun `client name runs through the same sanitize and cap path`() {
        ...
        val suffix = EnablingSequence.buildDeviceIdSuffix("Ü" + "z".repeat(40), "x", "y")
    ...
    @Test fun `non-ascii bytes are stripped before length-prefixing`() {
        ...
        val suffix = EnablingSequence.buildDeviceIdSuffix("bikeradar overlay", "Göogle", "Pixel™ Pro")
