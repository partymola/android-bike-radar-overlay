 ...
 */
class RadarStateProjectionTest {
    ...
    fun everySourceAdvertisesExactlyWhatItCanMeasure() {
        ...
        assertEquals(
            ...
            DataSource.entries.associateWith { RadarStateProjection.capabilitiesOf(it) },
        )
    }

    @Test
    fun aV2StreamAdvertisesEveryCapability() {
        assertEquals(15, RadarStateProjection.capabilitiesOf(DataSource.V2))
    }

    @Test
    fun aRangeOnlyStreamAdvertisesNothingItCannotMeasure() {
        assertEquals(0, RadarStateProjection.capabilitiesOf(DataSource.V1))
    }

    @Test
    fun noSourceAdvertisesNothing() {
        assertEquals(0, RadarStateProjection.capabilitiesOf(DataSource.NONE))
    }
    ...
    fun aTargetThatHasOvertakenTheRiderIsReportedAsAhead() {
        assertTrue(RadarStateProjection.toParcel(vehicle(isBehind = true), DataSource.V2).isAhead)
        assertFalse(RadarStateProjection.toParcel(vehicle(isBehind = false), DataSource.V2).isAhead)
    }

    @Test
    fun anUnusableLateralFrameIsReportedAsNotKnown() {
        assertFalse(RadarStateProjection.toParcel(vehicle(lateralUnknown = true), DataSource.V2).lateralKnown)
        assertTrue(RadarStateProjection.toParcel(vehicle(lateralUnknown = false), DataSource.V2).lateralKnown)
    }
    ...
    fun theBikeCodeIsReservedAndNeverProduced() {
        val emitted = VehicleSize.entries.map { RadarStateProjection.toParcel(vehicle(size = it), DataSource.V2).size }
        assertFalse("the radar reports no bikes, so the wire must not claim one", 2 in emitted)
        VehicleSize.entries.forEach { assertNotEquals(2, RadarStateProjection.toParcel(vehicle(size = it), DataSource.V2).size) }
    }

    @Test
    fun closingSpeedIsCarriedInKmhAndKeepsItsSign() {
        assertEquals(36, RadarStateProjection.toParcel(vehicle(speedMs = -10f), DataSource.V2).closingKmh)
        assertEquals(-18, RadarStateProjection.toParcel(vehicle(speedMs = 5f), DataSource.V2).closingKmh)
    }
    ...
    fun aRangeOnlyTrackProjectsAsDefaultsRatherThanMeasurements() {
        ...
        val p = RadarStateProjection.toParcel(v1, DataSource.V1)
    ...
    fun aRangeOnlyStateSaysItMeasuredNoneOfIt() {
        ...
        val p = RadarStateProjection.toParcel(state)
    ...
    fun aLiveV2StreamIsReportedAsDelivering() {
        ...
        assertTrue(RadarStateProjection.toParcel(RadarState(source = DataSource.V2)).streamLive)
    }
    ...
    fun targetsInOneFrameKeepTheirOwnFlags() {
        ...
        val out = RadarStateProjection.toParcel(state).vehicles
    ...
    fun aStreamWithNoLateralChannelReportsNoUsableLateralFrame() {
        ...
        assertFalse(
            ...
            RadarStateProjection.toParcel(v, DataSource.V1).lateralKnown,
        )
        assertTrue(RadarStateProjection.toParcel(v, DataSource.V2).lateralKnown)
    }
    ...
    fun aRiderSpeedWithoutTheCapabilityBehindItIsNotReportedAsKnown() {
        ...
        assertFalse(RadarStateProjection.toParcel(state).riderSpeedKnown)
    }
    ...
    fun everyTargetInAStateIsProjectedInOrder() {
        ...
        val empty = RadarStateProjection.toParcel(RadarState(vehicles = emptyList(), source = DataSource.V2))
        assertTrue("no targets on a live stream is a clear lane", empty.isClear)
    }

    @Test
    fun aStateCarriesItsOwnTimestamp() {
        val p = RadarStateProjection.toParcel(RadarState(timestamp = 1_234L, source = DataSource.V2))
    ...
    fun anAppWithNoRadarDoesNotReportAClearRoad() {
        ...
        val p = RadarStateProjection.toParcel(RadarState())
        ...
        val liveButClear = RadarStateProjection.toParcel(
    ...
    fun theRemainingFieldsAreCarriedThrough() {
        val p = RadarStateProjection.toParcel(vehicle(id = 7, distanceM = 42, lateralPos = -0.5f, rangeXm = -2.25f), DataSource.V2)
        assertEquals(7, p.id)
